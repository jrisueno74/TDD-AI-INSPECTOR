...
import { Building, AlertTriangle, CheckCircle2, Download, Loader2 } from 'lucide-react';
...
export function ReportView({ project, findings, onRestart }: Props) {
  ...
  useEffect(() => {
    async function fetchChecklist() {
      const result = await generateFinalChecklist(project, findings);
      setChecklist(result);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 print:space-y-0 print:max-w-none print:m-0">
      <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-gray-100 print:hidden">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Opciones de Informe</h2>
          <p className="text-xs text-gray-500 mt-1">Si el botón no funciona, abre la app en una nueva pestaña.</p>
        </div>
      ...
      <div id="pdf-content" className="space-y-8 print:space-y-6">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 print:border-none print:shadow-none print:p-0">
          <div className="mb-8 border-b pb-6">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Informe TDD Preliminar</h1>
            <div className="flex items-center gap-2 text-gray-500">
              <Building className="w-5 h-5" />
              <span className="text-lg">{project.name} • {project.type} ({project.year})</span>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="bg-red-50 p-4 rounded-xl border border-red-100">
            <div className="text-red-600 font-semibold mb-1">Incidencias Detectadas</div>
            <div className="text-3xl font-bold text-red-700">{incidences.length}</div>
          </div>
          <div className="bg-green-50 p-4 rounded-xl border border-green-100">
            <div className="text-green-600 font-semibold mb-1">Elementos OK</div>
            <div className="text-3xl font-bold text-green-700">{okFindings.length}</div>
          </div>
        </div>

        <div className="space-y-8">
          <h2 className="text-xl font-semibold text-gray-900 border-b pb-2">Detalle de Inspección</h2>
          {Object.entries(findingsByCategory).map(([category, catFindings]) => (
            <div key={category} className="space-y-4" style={{ pageBreakInside: 'avoid' }}>
              <h3 className="text-lg font-bold text-gray-800 bg-gray-100 px-3 py-2 rounded-md">{category}</h3>
                    ...
                    <div className="flex-1">
                      ...
                      {f.aiFeedback && (
                        <div className="bg-white/60 rounded-lg p-3 text-xs text-gray-700 border border-gray-200">
                          <span className="font-semibold block mb-1">Nota IA en campo:</span>
                          {f.aiFeedback}
                        </div>
                      )}
                    </div>
          ...
          {findings.length === 0 && (
            <p className="text-gray-500 text-sm">No se registraron hallazgos durante la inspección.</p>
          )}
        </div>
        ...
        <div className="bg-blue-50 p-8 rounded-2xl border border-blue-100 mt-8" style={{ pageBreakBefore: 'always' }}>
          <h2 className="text-xl font-semibold text-blue-900 mb-4 flex items-center gap-2">
            ...
            Notas para revisión en oficina (IA)
          </h2>
          {isGenerating ? (
            <div className="flex items-center gap-3 text-blue-700">
              <Loader2 className="w-5 h-5 animate-spin" />
              <span>Analizando inspección y normativas para sugerir comprobaciones finales...</span>
            </div>
          ) : (
