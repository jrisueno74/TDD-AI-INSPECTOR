...
import { generateFinalChecklist, generateUndetectedIncidencesSummary, generateExecutiveIncidencesSummary, ExecutiveIncidence, generateCapexEstimation, CapexItem } from '../services/geminiService';
...
export function ReportView({ project, findings, onRestart }: Props) {
  ...
  const [executiveSummary, setExecutiveSummary] = useState<ExecutiveIncidence[]>([]);
  const [capexEstimation, setCapexEstimation] = useState<CapexItem[]>([]);
  const [isGenerating, setIsGenerating] = useState(true);

  useEffect(() => {
    async function fetchChecklist() {
      const [result, undetected, executive, capex] = await Promise.all([
        generateFinalChecklist(project, findings, language),
        generateUndetectedIncidencesSummary(project, findings, language),
        generateExecutiveIncidencesSummary(project, findings, language),
        generateCapexEstimation(project, findings, language)
      ]);
      setChecklist(result);
      setUndetectedSummary(undetected);
      setExecutiveSummary(executive);
      setCapexEstimation(capex);
  ...
  const handleExportDocx = async () => {
    await exportToDocx(project, findings, checklist, undetectedSummary, executiveSummary, capexEstimation, t);
  };
